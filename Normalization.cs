...
namespace NS_ANN
{
    ...
    {
        public static void ShowMatrix(double[][] Matrix, int numSamples, int numCols)
        {
        ...
        public static Array Resize(Array old, int newSize)
        {
            Type elementType = old.GetType().GetElementType();
            Array _new = Array.CreateInstance(elementType, newSize);
            int minLength = Math.Min(old.Length, newSize);
            if(minLength > 0)
                Array.Copy(old, _new, minLength);
            return _new;
        }

        public static Array elemDel(Array curr, int index)
        {
            Array.ConstrainedCopy(curr, index + 1, curr, index, curr.Length - 1 - index);
            Resize(curr, curr.Length - 1);
            return curr;
        }
        ...
        public static void random(double[][] array)
        {
            ...
            Parallel.For(0, array.Length, j => { Parallel.For(0, array[j].Length, i => { array[j][i] = Rand.NextDouble(); }); });  
        }
    }
}